...
public class SQLiteManager {
	...
	public static void open(Context c) {
		InternalStorageManager.setContext(c);
	...
	public static void queryUser(Context context){
		/* Consultando para verificar se a inser��o est� no BD */
		Cursor cursor = database.query(MySQLiteHelper.TABLE_USUARIO,
				MySQLiteHelper.allColumnsUsuario, null, null, null, null, null);
		/* Mostrando o LOG */
		cursor.moveToFirst();
		Log.w("Banco de Dados", "Esta cadastrado:" + cursor.getLong(0)
				+ "o nome:" + cursor.getString(1));
		Toast.makeText(context,"Esta cadastrado:"+cursor.getString(1),Toast.LENGTH_SHORT).show();
		cursor.close();
	}
